"""
Download and prepare training data for the Spherical Voronoi Gaussian Splatting.

Identical data pipeline to the sibling `3dgs` project: the Mip-NeRF 360 release
ships the exact COLMAP reconstruction the paper trains from, so we download the
dataset, extract one scene, and parse the COLMAP `sparse/0` binaries into:
