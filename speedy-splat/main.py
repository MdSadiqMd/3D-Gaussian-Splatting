...
class PruneGradAccum(torch.autograd.Function):
    """Accumulates (dL/dg_i)² per Gaussian during backward.

    In forward: passes g through unchanged.
    In backward: for each Gaussian i, adds Σ_pixels (dL/dg_i(p))² to accum[global_id].
    """

    @staticmethod
    def forward(ctx, g, local_ids, idx_map, accum):
    ...
    def backward(ctx, grad_output):
        if ctx.accum is not None and ctx.idx_map is not None:
            sq = (grad_output.detach() ** 2).sum(dim=1)
            global_ids = ctx.idx_map[ctx.saved_tensors[0]]
            ctx.accum.scatter_add_(0, global_ids, sq)
...
def ellipse_tile_intersects(ic, u, v, t, tile_x0, tile_x1, tile_y0, tile_y1):
    """AccuTile: check if ellipse (center u,v, inverse cov ic, threshold t)
    intersects the tile [tile_x0, tile_x1] x [tile_y0, tile_y1]."""
    a, b, c = ic[0, 0], ic[0, 1], ic[1, 1]
    xs = torch.tensor([tile_x0, tile_x1], device=u.device, dtype=u.dtype)
    ys = torch.tensor([tile_y0, tile_y1], device=u.device, dtype=u.dtype)
    grid_x, grid_y = torch.meshgrid(xs, ys, indexing="xy")
    dx = grid_x.reshape(-1) - u
    dy = grid_y.reshape(-1) - v
    q = a * dx * dx + 2 * b * dx * dy + c * dy * dy
    any_inside = (q <= t).any()
    if any_inside:
        return True
    edges_x = torch.tensor([tile_x0, tile_x1], device=u.device, dtype=u.dtype)
    for ex in edges_x:
        dy_range = torch.sqrt(torch.clamp(t / c, min=0.0))
        y_min_ell = v - dy_range
        y_max_ell = v + dy_range
        if y_min_ell <= tile_y1 and y_max_ell >= tile_y0:
            dx_e = ex - u
            rem = t - a * dx_e * dx_e
            if rem >= 0:
                dy_e = torch.sqrt(rem / c)
                y_e_low = v - dy_e
                y_e_high = v + dy_e
                if y_e_low <= tile_y1 and y_e_high >= tile_y0:
                    return True
    return False
...
def render(
    ...
    major_variance = (mid + disc).clamp_min(1e-12).clamp_max(1e4)
    ...
    snug_t = 2.0 * torch.log(255.0 * opacity + 1e-10)
    snug_det = ic00 * ic11 - ic[:, 0, 1] * ic[:, 1, 0]

    y_half = torch.sqrt(torch.clamp(snug_t * ic00 / snug_det, min=0.0))
    x_half = torch.sqrt(torch.clamp(snug_t * ic11 / snug_det, min=0.0))

    snug_radius = torch.ceil(3.0 * torch.sqrt(major_variance)).to(torch.int64)
    umin = torch.floor(u - torch.min(x_half, snug_radius.float())).to(torch.int64)
    umax = torch.floor(u + torch.min(x_half, snug_radius.float())).to(torch.int64)
    vmin = torch.floor(v - torch.min(y_half, snug_radius.float())).to(torch.int64)
    vmax = torch.floor(v + torch.min(y_half, snug_radius.float())).to(torch.int64)
    ...
    for tile_id, s0, s1 in zip(unique_tile_ids.tolist(), start.tolist(), end.tolist()):
        ...
        gaussian_i_ic = inverse_covariance[current_gaussian_ids]

        du = px_u.unsqueeze(0) - gaussian_i_u.unsqueeze(-1)
        dv = px_v.unsqueeze(0) - gaussian_i_v.unsqueeze(-1)
        A11 = gaussian_i_ic[:, 0, 0].unsqueeze(-1)
        A12 = gaussian_i_ic[:, 0, 1].unsqueeze(-1)
        A22 = gaussian_i_ic[:, 1, 1].unsqueeze(-1)
        ...
        if pruning_accum is not None:
            g = PruneGradAccum.apply(g, current_gaussian_ids, idx, pruning_accum)
    ...
    info = {"idx": idx, "means2d": means2d}
    return image, info


def rgb_to_sh_dc(rgb):
    rgb = rgb.clamp(1e-4, 1 - 1e-4)
    logit = torch.log(rgb / (1 - rgb))
    return logit / SH_C0
...
class GaussianModel:
    def __init__(self, xyz, rgb, extent, device):
        ...
        q_raw[:, 3] = 1.0
        opacity_raw = torch.full((n,), inverse_sigmoid(0.1))
...
def densify_and_prune(
    ...
    if n_split > 0:
        ...
        split = {
            ...
            "q_raw": q,
            "opacity_raw": gm.opacity_raw[split_mask].repeat(2),
...
@torch.no_grad()
def soft_prune(gm, pruning_accum, prune_ratio=0.8):
    """Speedy-Splat Soft Pruning via (∇_{g_i} L)².
    Removes prune_ratio fraction of gaussians with lowest accumulated score."""
    n = pruning_accum.shape[0]
    k = max(1, int(n * (1 - prune_ratio)))
    _, keep_idx = torch.topk(pruning_accum, k)
    mask_keep = torch.zeros(n, dtype=torch.bool, device=pruning_accum.device)
    mask_keep[keep_idx] = True
    gm.replace(mask_keep, None)


@torch.no_grad()
def hard_prune(gm, pruning_accum, prune_ratio=0.3):
    """Speedy-Splat Hard Pruning via (∇_{g_i} L)²."""
    n = pruning_accum.shape[0]
    k = max(1, int(n * (1 - prune_ratio)))
    _, keep_idx = torch.topk(pruning_accum, k)
    mask_keep = torch.zeros(n, dtype=torch.bool, device=pruning_accum.device)
...
def train(
    ...
    hard_prune_from,
    hard_prune_interval,
    soft_prune_ratio=0.8,
    hard_prune_ratio=0.3,
    lambda_dssim=0.2,
):
    ...
    for it in pbar:
        ...
        if it in soft_prune_iters:
            n_before = gm.pos.shape[0]
            soft_prune(gm, pruning_accum, prune_ratio=soft_prune_ratio)
            n_after = gm.pos.shape[0]
            print(f"  Soft prune at iter {it}: {n_before} -> {n_after} gaussians")
            optimizer = gm.make_optimizer(spatial_lr_scale=extent)
            grad_accum = torch.zeros(gm.pos.shape[0], device=device)
            denom = torch.zeros(gm.pos.shape[0], device=device)
            pruning_accum = torch.zeros(gm.pos.shape[0], device=device)

        if hard_prune_from and it >= hard_prune_from and it % hard_prune_interval == 0:
            n_before = gm.pos.shape[0]
            hard_prune(gm, pruning_accum, prune_ratio=hard_prune_ratio)
            n_after = gm.pos.shape[0]
            print(f"  Hard prune at iter {it}: {n_before} -> {n_after} gaussians")
            optimizer = gm.make_optimizer(spatial_lr_scale=extent)
            grad_accum = torch.zeros(gm.pos.shape[0], device=device)
            denom = torch.zeros(gm.pos.shape[0], device=device)
            pruning_accum = torch.zeros(gm.pos.shape[0], device=device)
...
if __name__ == "__main__":
    ...
    gm = train(
        ...
        soft_prune_iters=[6000, 9000, 12000],
        hard_prune_from=15000,
        hard_prune_interval=3000,
        soft_prune_ratio=0.8,
        hard_prune_ratio=0.3,
    )
