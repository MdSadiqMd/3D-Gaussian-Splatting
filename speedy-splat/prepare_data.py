"""
Download and prepare training data for Speedy-Splat.

Parses the Mip-NeRF 360 COLMAP reconstruction (same initialization as 3D-GS)
into np arrays and torch tensors:

  out_colmap/<scene>/cam_meta.npy     full-res pinhole intrinsics
  out_colmap/<scene>/cameras.npy      per-training-image {name, c2w}
  out_colmap/<scene>/points3D.npy     SfM point cloud {xyz, rgb}
  camera_trajectories/<scene>_orbit.pt  smooth novel-view path
"""
...
def main():
    ...
    if cam["model_id"] == 1:
        ...
        raise SystemExit(
            f"Expected PINHOLE camera, got model_id={cam['model_id']}. "
            "Mip-NeRF 360 ships undistorted images."
        )
